...
test('grades e2e: create assessment -> enter grade -> prefill on reopen', async ({ page, request }) => {
  ...
  await expect(page.getByText(/grade entry/i)).toBeVisible({ timeout: 20000 });
  ...
  await expect(gradeInputs.first()).toBeVisible({ timeout: 30000 });
  // set first student's grade
  await gradeInputs.first().fill('97');
