...
echo "==> Applying UI upgrade v2 (no perl): overwrite needed files"

mkdir -p lib/features/{home,schedule,classrooms,attendance,grades,announcements,assignments} lib/app

# --- Home shell (centered ClassMate + Drawer + bottom nav includes Classrooms)
    ...
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: const [
          ...
          NavigationDestination(
              icon: Icon(Icons.class_outlined), label: 'Classrooms'),
          NavigationDestination(
              icon: Icon(Icons.smart_toy_outlined), label: 'Tutor'),
          NavigationDestination(
        ...
        child: ListView(
          ...
          children: [
            ...
            ListTile(
              ...
              title: const Text('Import (Google Classroom demo)',
                  style: TextStyle(fontWeight: FontWeight.w800)),
              onTap: () {
                Navigator.pop(context);
                context.push('/import');
...
# --- Schedule (day pager + "Your classes" buttons)
...
class _ScheduleScreenState extends State<ScheduleScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final title = DateFormat("EEEE - d/M/yyyy").format(selected);
            ...
            child: ListTile(
              ...
              onTap: () {
                final match = DemoStore.classrooms.firstWhere(
                  (c) => x.title.toLowerCase().contains(
                      c.name.toLowerCase().split(' ').first.toLowerCase()),
...
# --- Classroom detail (chat/assignments/meetings/solutions)
...
class _ClassroomDetailScreenState extends State<ClassroomDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final assigns =
        DemoStore.assignments.where((a) => a.classroomId == c.id).toList();
                    ...
                    children: [
                      IconButton(
                        onPressed: () => ScaffoldMessenger.of(context)
                            .showSnackBar(const SnackBar(content: Text('Photo sending: demo'))),
                        icon: const Icon(Icons.photo_outlined),
                      ),
                      IconButton(
                        onPressed: () => ScaffoldMessenger.of(context)
                            .showSnackBar(const SnackBar(content: Text('Voice message: demo'))),
                    ...
                    child: ListTile(
                      ...
                      onTap: () {
                        ...
                        ScaffoldMessenger.of(context)
                            .showSnackBar(SnackBar(content: Text('Submitted. +${a.reward} points')));
                      },
                    ),
                  ),
              ],
                ...
                Card(
                  child: ListTile(
                    ...
                    onTap: () => ScaffoldMessenger.of(context)
                        .showSnackBar(const SnackBar(content: Text('Posting: demo'))),
                  ),
                ),
                Card(
                  child: ListTile(
