...
NET="$(docker inspect "$API_CID" -f '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}')"
if [ -z "$NET" ]; then
...
docker run --rm --network "$NET" curlimages/curl:8.10.1 -fsS   --retry 30 --retry-connrefused --retry-delay 1 --max-time 10   http://api:3000/api/health >/dev/null
