...
Monorepo: `services/api` (NestJS + Prisma + Postgres, on Railway) · `apps/classmate_mobile` (Flutter — iOS, Android, Web) · `marketing/` (static site). AI tutor "NOVA" is Anthropic Codex (students-only). Stack also: R2, Resend, Twilio, RevenueCat, Firebase, Sentry.
